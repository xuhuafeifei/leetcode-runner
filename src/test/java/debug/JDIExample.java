...
public class JDIExample {
                ...
                for (Event event : eventSet) {
                    if (event instanceof ClassPrepareEvent) {
                        ...
                        if (className.equals("Main")) {
                            ...
                            // 设置断点
                            // 在指定行设置断点
                            setBreakpointAtLine(mainClass, 10, erm);
                            System.out.println("Breakpoint set at Main.main");
                        }
                        if (className.equals("Solution")) {
                            // 获取Main类
                            ClassType mainClass = (ClassType) classPrepareEvent.referenceType();
                            Method mainMethod = mainClass.methodsByName("findMedianSortedArrays").get(0);
    ...
    // 在指定行设置断点
    private static void setBreakpointAtLine(ClassType mainClass, int lineNumber, EventRequestManager erm) {
        try {
            // 获取方法
            Method mainMethod = mainClass.methodsByName("main").get(0);
            // 获取方法的所有位置
