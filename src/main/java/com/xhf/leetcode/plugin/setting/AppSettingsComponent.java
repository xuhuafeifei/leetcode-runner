...
public class AppSettingsComponent {
    ...
    public AppSettingsComponent() {
        ...
        myMainPanel = FormBuilder.createFormBuilder()
            .addLabeledComponent(new JBLabel(BundleUtils.i18nHelper("编程语言", "Lang  type")),
                InnerHelpTooltip.FlowLayout(FlowLayout.LEFT).add(myLangType).addHelp(LANG_TYPE_HELP_TEXT)
                    .getTargetComponent(), 1, false)
            .addLabeledComponent(new JBLabel(BundleUtils.i18nHelper("存储路径", "Store path")),
                InnerHelpTooltip.BoxLayout().add(myFileBrowserBtn).addHelp(STORE_PATH_HELP_TEXT).getTargetComponent(),
                1,
                false)
            ...
            .addComponent((JComponent) Box.createVerticalStrut(5))
            .addLabeledComponent(new JBLabel(BundleUtils.i18nHelper("重定位", "Reposition")),
                InnerHelpTooltip.FlowLayout(FlowLayout.LEFT).add(reposition).addHelp(REPOSITION_HELP_TEXT)
                    .getTargetComponent(), 1, false)
            .addLabeledComponent(new JBLabel(BundleUtils.i18nHelper("语言和地区", "Language  ")),
                InnerHelpTooltip.FlowLayout(FlowLayout.LEFT).add(language).addHelp(LANGUAGE_HELP_TEXT)
                    .getTargetComponent(),
                1, false)
            .addComponent(createEncryptPanel())
            .addLabeledComponent(
                new JBLabel(BundleUtils.i18nHelper("是否启用悬浮工具栏", "enable floating toolbar or not")),
                InnerHelpTooltip.FlowLayout(FlowLayout.LEFT).add(enableFloatingToolbarBtn)
                    .addHelp(FLOATING_TOOLBAR_HELP_TEXT).getTargetComponent(), 1, false)
    ...
    private void initComponent() {
        ...
        reposition.addItem(
            BundleUtils.i18nHelper(LanguageConvertor.REPOSITION_DEFAULT_ZH, LanguageConvertor.REPOSITION_DEFAULT_EN));
        reposition.addItem(
            BundleUtils.i18nHelper(LanguageConvertor.REPOSITION_SETTING_ZH, LanguageConvertor.REPOSITION_SETTING_EN));
        ...
        language.addActionListener(e -> {
            ...
            if (StringUtils.isNotBlank(selectedItem) && !selectedItem.equals(AppSettings.getInstance().getLocale())) {
                ViewUtils.getDialogWrapper(BundleUtils.i18nHelper("语言设置生效需要重启IDE, 请您保存设置并重启",
                    "Language settings take effect after restarting IDE, please save the settings and restart"));
            }
        });
    ...
    public void setEnableFloatingToolbar(boolean enableFloatingToolbar) {
        ...
        enableFloatingToolbarBtn.dispatchEvent(
            new ActionEvent(enableFloatingToolbarBtn, ActionEvent.ACTION_PERFORMED, null));
    }
