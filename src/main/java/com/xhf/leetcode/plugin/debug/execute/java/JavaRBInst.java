        ...
        for (BreakpointRequest breakpointRequest : breakpointRequests) {
            if (breakpointRequest.location().lineNumber() == lineNumber) {
                ...
                ExecuteResult success = ExecuteResult.success(inst.getOperation(),
                    BundleUtils.i18nHelper("断点已在" + lineNumber + "行移除!",
                        "break point remove at line " + lineNumber));
                success.setAddLine(lineNumber);
                return success;
            }
        }
        return ExecuteResult.fail(inst.getOperation(), BundleUtils.i18nHelper("断点在" + lineNumber + "行不存在!",
            "break point at " + lineNumber + " haven't been set!"));
    }
}
