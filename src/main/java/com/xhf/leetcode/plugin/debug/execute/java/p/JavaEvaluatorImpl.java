...
//            v = takeValueByVName(vName, Env.getStaticValues());
        ...
        protected boolean checkMethodExist(String vName, String methodName, Value v) {
            if (! (v instanceof ObjectReference) || ((ObjectReference) v).referenceType().methodsByName(methodName).isEmpty()) {
                throw new DebugError(vName + " " + BundleUtils.i18n("debug.eval.not.exist") + " " + methodName + BundleUtils.i18n("debug.eval.method"));
            }
            return true;
        }
        ...
        protected Value handleWrapperAndPrimitive(Value v, Type type) throws ClassNotLoadedException, IncompatibleThreadStateException, InvocationException, InvalidTypeException {
            ...
            throw new ComputeError(BundleUtils.i18n("debug.eval.method.error.variable") + v.type().name() + " type = " + type.name());
        }
    ...
    public static class EvalToken extends AbstractToken {
        ...
        public Value getValue() {
            ...
            switch (className) {
                ...
                default:
                    throw new ComputeError(BundleUtils.i18n("debug.eval.unrecognize.result.type") + execute + " " + className);
            }
            try {
    ...
    public static class InvokeToken extends AbstractToken {
        ...
        public Method getMethod(Value callV, InvokeInfo info) {
            ...
            if (! checkMethodExist(callVName, methodName, callV)) {
                throw new ComputeError(callVName + " " + BundleUtils.i18n("debug.eval.not.exist") + " " + methodName + " " + BundleUtils.i18n("debug.eval.method"));
            }
            // 判断重载
        ...
        private Method overloadCheck(Value callV, InvokeInfo info) {
            ...
            if (candidates.isEmpty()) {
                throw new ComputeError(callVName + " " + BundleUtils.i18n("debug.eval.no.applicable.for.exp") + " " + methodName + " " + BundleUtils.i18n("debug.eval.method"));
            }
            // 如果只有一个
        ...
        private void matchValueAndType(Method targetMethod, InvokeInfo info) throws ClassNotLoadedException, IncompatibleThreadStateException, InvocationException, InvalidTypeException {
            ...
            if (!types.isEmpty() && (paramNames == null || paramNames.length != types.size()) ) {
                throw new ComputeError(BundleUtils.i18n("debug.eval.method.parse.error") + info + " " + BundleUtils.i18n("debug.eval.actual.param") + types.stream().map(Type::name).collect(Collectors.joining(",")));
            }
            ...
            if (! checkConsist(v, type)) {
                if (checkWrapperAndPrimitive(v, type)) {
                    ...
                    if (v instanceof PrimitiveValue) {
                        ...
                        throw new ComputeError(BundleUtils.i18n("debug.eval.variable.type.error") + vName + BundleUtils.i18n("debug.eval.type") + v.type().name() + ", " + BundleUtils.i18n("debug.eval.input.type") + " " + type.name());
                    }
                } else {
                    // 报错
                    throw new ComputeError(BundleUtils.i18n("debug.eval.variable.type.error") + vName + BundleUtils.i18n("debug.eval.type") + v.type().name() + ", " + BundleUtils.i18n("debug.eval.input.type") + " " + type.name());
        ...
        private List<Value> getParams(String[] paramNames) {
            ...
            for (int i = 0; i < paramNames.length; i++) {
                try {
                    ...
                    LogUtils.warn(DebugUtils.getStackTraceAsString(e));
                    throw new ComputeError(paramNames[i] + " " + BundleUtils.i18n("debug.eval.parse.error") + "! cause is " + e.getMessage());
                }
            }
            return values;
        }
    ...
    public static class PureCallTokenChain extends PureCallToken implements TokenChain {
        ...
        private String getPureCall(String token) {
            ...
            if (! matcher.find()) {
                throw new ComputeError("PureCallTokenChain" + BundleUtils.i18n("debug.eval.unrecognize") + " token: " + token);
            }
            int end = matcher.end();
        ...
        public Value getValue(@NotNull PValue pValue) {
            if (pValue.value == null) {
                throw new ComputeError(BundleUtils.i18n("debug.eval.chain.pre.call") + pValue.preName + " = null, " + BundleUtils.i18n("debug.eval.chain.exp.error"));
            }
            String pureCall = getPureCall(token);
            ...
            if (! (pV instanceof ObjectReference) ) {
                throw new ComputeError(BundleUtils.i18n("debug.eval.chain.exp.error") + " " + BundleUtils.i18n("debug.eval.only.object.has.method") + "  pV = " + pV.type().name());
            }
            ...
            for (int i = 0; i < dims.size(); i++) {
                ...
                if (dim >= array.length()) {
                    if (I18nTypeEnum.getI18N(AppSettings.getInstance().getLocale()) == I18nTypeEnum.ZH) {
                        ...
                        throw new ComputeError("Array " + arrayId + " out of bounds!! The number of elements in the " + (i + 1) + " dimension of the array=" + array.length() + ", index=" + dim);
                    }
                }
                if (dim <= -1) {
                    if (I18nTypeEnum.getI18N(AppSettings.getInstance().getLocale()) == I18nTypeEnum.ZH) {
                ...
                if (i != dims.size() - 1) {
                    if (!(value instanceof ArrayReference)) {
                        if (I18nTypeEnum.getI18N(AppSettings.getInstance().getLocale()) == I18nTypeEnum.ZH) {
                            ...
                            throw new ComputeError("Variable " + arrayId + " dimension " + i + " is not an array type, it is " + value.type().name() + " type!!");
                        }
                    }
                    // 跟新array
        ...
        public Value getValue(PValue pValue) {
            if (pValue.value == null) {
                if (I18nTypeEnum.getI18N(AppSettings.getInstance().getLocale()) == I18nTypeEnum.ZH) {
                    ...
                    throw new ComputeError("The call before the chain call " + pValue.preName + " is null, chain expression error!");
                }
            }
            boolean flag = isImplicitCall(token);
            ...
            if (flag) {
                if (! (pV instanceof ArrayReference) ) {
                    if (I18nTypeEnum.getI18N(AppSettings.getInstance().getLocale()) == I18nTypeEnum.ZH) {
                        ...
                        throw new ComputeError("pV is not Array type! Expression calculation result error! type = " + pV.type().name());
                    }
                }
            } else {
                // 非隐式调用, 从pV中取值
                ...
                if (c == '"') {
                    ...
                    if (j == arr.length) {
                        if (I18nTypeEnum.getI18N(AppSettings.getInstance().getLocale()) == I18nTypeEnum.ZH) {
                            ...
                            throw new ComputeError("Expression error, the string type expression must contain a matching \"" + s);
                        }
                    }
                    // j 指向", 因此需要j + 1
                    ...
                    if (j == arr.length) {
                        if (I18nTypeEnum.getI18N(AppSettings.getInstance().getLocale()) == I18nTypeEnum.ZH) {
                            ...
                            throw new ComputeError("Expression error, the character type expression must contain a matching ' " + s);
                        }
                    }
                    // j指向', 因此需要j + 1
                    return s.substring(0, j + 1);
                        ...
                        if (! CharacterHelper.isArabicNumber(arr[j])) {
                            ...
                            if (arr[j] != '.') {
                                ...
                                if (I18nTypeEnum.getI18N(AppSettings.getInstance().getLocale()) == I18nTypeEnum.ZH) {
                                    ...
                                    throw new ComputeError("Expression error, the numeric type expression is not allowed to appear two decimal points " + s);
                                }
                            }
                        }
                        j += 1;
        ...
        public Token parseToToken(String s, Context ctx) {
            ...
            if (matches.size() > 1) {
                if (I18nTypeEnum.getI18N(AppSettings.getInstance().getLocale()) == I18nTypeEnum.ZH) {
                    ...
                    throw new ComputeError(s + " is matched by multiple rules! Please check whether the rule writing is correct! + " + matches.stream().map(Rule::getName).collect(Collectors.joining(",")));
                }
            }
            ...
            try {
                ...
                if (I18nTypeEnum.getI18N(AppSettings.getInstance().getLocale()) == I18nTypeEnum.ZH) {
                    ...
                    throw new ComputeError(className + "Instantiation error, the object is an abstract class and cannot be instantiated! " + e.getMessage());
                }
                ...
                if (I18nTypeEnum.getI18N(AppSettings.getInstance().getLocale()) == I18nTypeEnum.ZH) {
                    ...
                    throw new ComputeError(className + "Constructor access error, String.class, Context.class Constructor has no access! " + e.getMessage());
                }
                ...
                if (I18nTypeEnum.getI18N(AppSettings.getInstance().getLocale()) == I18nTypeEnum.ZH) {
                    ...
                    throw new ComputeError(className + "Constructor execution exception, an error occurred during the execution of String.class, Context.class! " + e.getMessage());
                }
                ...
                if (I18nTypeEnum.getI18N(AppSettings.getInstance().getLocale()) == I18nTypeEnum.ZH) {
                    ...
                    throw new ComputeError(className + "There is no constructor that matches String.class, Context.class! " + e.getMessage());
                }
            }
        }
        ...
        public Token parseToTokenFromStart(String token, Context ctx) {
                ...
                if (hits.stream().anyMatch(e -> e.rule instanceof EvalRule) ) {
                    ...
                    if (targetHit == null) {
                        if (I18nTypeEnum.getI18N(AppSettings.getInstance().getLocale()) == I18nTypeEnum.ZH) {
                            ...
                            throw new ComputeError(s + " is matched by EvalRule twice! Please check the rule writing is correct! + " + hits.stream().map(e -> e.rule.getName()).collect(Collectors.joining(",")));
                        }
                    }
                } else  {
                    if (I18nTypeEnum.getI18N(AppSettings.getInstance().getLocale()) == I18nTypeEnum.ZH) {
                        ...
                        throw new ComputeError(s + " is matched by two rules! Please check the rule writing is correct! + " + hits.stream().map(e -> e.rule.getName()).collect(Collectors.joining(",")));
                    }
                }
            } else {
                if (I18nTypeEnum.getI18N(AppSettings.getInstance().getLocale()) == I18nTypeEnum.ZH) {
                    ...
                    throw new ComputeError(s + " is matched by multiple rules! Please check the rule writing is correct! + " + hits.stream().map(e -> e.rule.getName()).collect(Collectors.joining(",")));
                }
            }
            ...
            try {
                ...
                if (I18nTypeEnum.getI18N(AppSettings.getInstance().getLocale()) == I18nTypeEnum.ZH) {
                    ...
                    throw new ComputeError(className + "instantiation error, the object is an abstract class, cannot be instantiated! " + e.getMessage());
                }
            } catch (IllegalAccessException e) {
                LogUtils.error(e);
                if (I18nTypeEnum.getI18N(AppSettings.getInstance().getLocale()) == I18nTypeEnum.ZH) {
                ...
                if (I18nTypeEnum.getI18N(AppSettings.getInstance().getLocale()) == I18nTypeEnum.ZH) {
                    ...
                    throw new ComputeError(className + "Constructor execution exception, String.class, Context.class is running and an error occurs! " + e.getMessage());
                }
            } catch (NoSuchMethodException e) {
                LogUtils.error(e);
                if (I18nTypeEnum.getI18N(AppSettings.getInstance().getLocale()) == I18nTypeEnum.ZH) {
                    ...
                    throw new ComputeError(className + "does not have a constructor that matches String.class, Context.class! " + e.getMessage());
                }
            }
        }
        ...
        public TokenChain parseToTokenChain(String s, Context ctx) {
            ...
            if (matches.size() > 1) {
                if (I18nTypeEnum.getI18N(AppSettings.getInstance().getLocale()) == I18nTypeEnum.ZH) {
                    ...
                    throw new ComputeError(s + " is matched by multiple rules! Please check the rule writing is correct! + " + matches.stream().map(Rule::getName).collect(Collectors.joining(",")));
                }
            }
            // 如果包含EvalRule, 报错
            if (matches.stream().anyMatch(e -> e instanceof EvalRule)) {
                if (I18nTypeEnum.getI18N(AppSettings.getInstance().getLocale()) == I18nTypeEnum.ZH) {
                    ...
                    throw new ComputeError(s + " is matched by EvalRule! Cannot get the corresponding chain! parseToTokenChain method error! + " + matches.stream().map(Rule::getName).collect(Collectors.joining(",")));
                }
            }
            Rule targetRule;
            // 规则转换
            if (! (matches.get(0) instanceof RuleChain) ) {
                ...
                if (!ruleMap.containsKey(key)) {
                    if (I18nTypeEnum.getI18N(AppSettings.getInstance().getLocale()) == I18nTypeEnum.ZH) {
                        ...
                        throw new ComputeError(s + " is matched by " + key + "! Cannot get the corresponding chain! parseToTokenChain method error! + " + matches.stream().map(Rule::getName).collect(Collectors.joining(",")));
                    }
                }
            ...
            try {
                ...
                if (I18nTypeEnum.getI18N(AppSettings.getInstance().getLocale()) == I18nTypeEnum.ZH) {
                    ...
                    throw new ComputeError(className + "Constructor access error, String.class, Context.classConstructor cannot be accessed! " + e.getMessage());
                }
            } catch (InvocationTargetException e) {
                LogUtils.error(e);
                if (I18nTypeEnum.getI18N(AppSettings.getInstance().getLocale()) == I18nTypeEnum.ZH) {
                    ...
                    throw new ComputeError(className + "Constructor execution exception, an error occurred during the running of String.class, Context.class! " + e.getMessage());
                }
            } catch (NoSuchMethodException e) {
                LogUtils.error(e);
                if (I18nTypeEnum.getI18N(AppSettings.getInstance().getLocale()) == I18nTypeEnum.ZH) {
