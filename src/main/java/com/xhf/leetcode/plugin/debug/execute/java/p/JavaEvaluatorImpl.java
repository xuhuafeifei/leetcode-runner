...
public class JavaEvaluatorImpl implements Evaluator {
     ...
     */
    // static别删, 不然IDE有时候抽风会报错, 找不到Token
    ...
    static class Env {
        ...
        public static Map<String, Value> takeMemberValues(Value v) {
            ...
            if (!(v instanceof ObjectReference)) {
                throw new ComputeError(BundleUtils.i18n("debug.eval.method.use.error") + v.type());
            }
            ObjectReference objRef = (ObjectReference) v;
            List<Field> fields = objRef.referenceType().fields().stream().filter(e -> !e.isStatic())
         ...
         */
        private @NotNull
        static Map<String, Value> takeStaticValues(ReferenceType referenceType) {
         ...
         */
        private @NotNull
        static Map<String, Value> takeStaticValues(Value v) {
            if (v == null) {
                return MapUtils.emptyMap();
            }
            if (v instanceof ObjectReference) {
    ...
    public abstract static class AbstractToken implements Token {
        ...
        public static void doCheck(String expression) {
            ...
            if (!areParenthesesAndBracketsBalanced(expression)) {
                throw new ComputeError(BundleUtils.i18n("debug.eval.brackets.error"));
            }
            // 检查是否存在+=等赋值语句
            ...
            if (expression.contains("++") || expression.contains("--")) {
                throw new ComputeError(BundleUtils.i18n("debug.eval.auto.inc.dec"));
            }
        }
        ...
        protected Value takeValueByVName(String vName) {
            ...
            if (!v.contain) {
                throw new ComputeError(vName + " " + BundleUtils.i18n("debug.eval.not.define"));
            }
            return v.value;
        }
        ...
        protected boolean checkMethodExist(String vName, String methodName, Value v) {
            if (!(v instanceof ObjectReference) || ((ObjectReference) v).referenceType().methodsByName(methodName)
                .isEmpty()) {
                throw new DebugError(
                    vName + " " + BundleUtils.i18n("debug.eval.not.exist") + " " + methodName + BundleUtils.i18n(
                        "debug.eval.method"));
            }
            return true;
        }
         ...
         * @return 类型转换
         */
        protected Value handleWrapperAndPrimitive(Value v, Type type)
            ...
            throw new ComputeError(
                BundleUtils.i18n("debug.eval.method.error.variable") + v.type().name() + " type = " + type.name());
        }
        ...
        protected Value WrapperObjReferenceToPrimitive(Value value) {
            if (value instanceof ObjectReference) {
                ...
                if (Env.inspector.isWrapperType(objRef)) {
                    ...
                    throw new ComputeError(BundleUtils.i18n("debug.eval.only.support.wrapper") + value.type().name());
                }
            }
            throw new ComputeError(BundleUtils.i18n("debug.eval.only.support.wrapper") + value.type().name());
        }
         ...
         */
        protected Value PrimitiveValueToWrapperObjReference(Value value)
            throws ClassNotLoadedException, IncompatibleThreadStateException, InvocationException, InvalidTypeException {
            if (value instanceof PrimitiveValue) {
                ...
                Value v = wrapperClass.newInstance(ctx.getThread(), constructor,
                    Collections.singletonList(primitiveValue), 0);
                ctx.invokeMethodDone();
                return v;
            }
            throw new RuntimeException(BundleUtils.i18n("debug.eval.only.support.primitive") + value.type().name());
        }
    ...
    public static class EvalToken extends AbstractToken {
        ...
        public Value getValue() {
            ...
            switch (className) {
                ...
                default:
                    throw new ComputeError(
                        BundleUtils.i18n("debug.eval.unrecognize.result.type") + execute + " " + className);
            }
            try {
        ...
        public static class TypeChecker {
            ...
            static {
                ...
                operatorTypeRules.put("-",
                    Collections.singletonList((Arrays.asList(TypeEnum.NUMBER, TypeEnum.NUMBER, TypeEnum.NUMBER))));
                operatorTypeRules.put("*",
                    Collections.singletonList((Arrays.asList(TypeEnum.NUMBER, TypeEnum.NUMBER, TypeEnum.NUMBER))));
                operatorTypeRules.put("/",
                    Collections.singletonList((Arrays.asList(TypeEnum.NUMBER, TypeEnum.NUMBER, TypeEnum.NUMBER))));
                ...
                operatorTypeRules.put("^",
                    Collections.singletonList((Arrays.asList(TypeEnum.NUMBER, TypeEnum.NUMBER, TypeEnum.NUMBER))));
                operatorTypeRules.put("~",
                    Collections.singletonList((List.of(TypeEnum.NUMBER, TypeEnum.NUMBER)))); // 单目运算符
                ...
                operatorTypeRules.put("<",
                    Collections.singletonList((Arrays.asList(TypeEnum.NUMBER, TypeEnum.NUMBER, TypeEnum.BOOLEAN))));
                operatorTypeRules.put(">",
                    Collections.singletonList((Arrays.asList(TypeEnum.NUMBER, TypeEnum.NUMBER, TypeEnum.BOOLEAN))));
                operatorTypeRules.put("<=",
                    Collections.singletonList((Arrays.asList(TypeEnum.NUMBER, TypeEnum.NUMBER, TypeEnum.BOOLEAN))));
                ...
                operatorTypeRules.put("&&",
                    Collections.singletonList((Arrays.asList(TypeEnum.BOOLEAN, TypeEnum.BOOLEAN, TypeEnum.BOOLEAN))));
                operatorTypeRules.put("||",
                    Collections.singletonList((Arrays.asList(TypeEnum.BOOLEAN, TypeEnum.BOOLEAN, TypeEnum.BOOLEAN))));
                operatorTypeRules.put("!",
                    Collections.singletonList(List.of(TypeEnum.BOOLEAN, TypeEnum.BOOLEAN))); // 单目运算符
            }
                ...
                for (Type type : typeList) {
                    if (type.typeEnum == TypeEnum.OPERATOR) {
                        ...
                        if (operandStack.size() < 2 && !isUnaryOperator(type.token)) {
                            throw new ComputeError(
                                BundleUtils.i18nHelper("操作数不足", "Insufficient operands for operator: ")
                                    + type.token);
                        }
                        ...
                        if (isUnaryOperator(type.token)) {
                            ...
                            if (validOperation == null) {
                                throw new ComputeError(
                                    BundleUtils.i18nHelper("类型不匹配", "Invalid types for operator '")
                            ...
                            if (validOperation == null) {
                                throw new ComputeError(BundleUtils.i18nHelper(
                                    String.format("类型不匹配, 运算符'%s'不支持操作类型'%s'和'%s'", type.token,
                                        left.typeEnum, right.typeEnum),
                                    String.format("Invalid types for operator '%s': %s and %s", type.token,
                                        left.typeEnum, right.typeEnum)
                                ));
                            }
                            operandStack.push(validOperation.get(validOperation.size() - 1).toType()); // 压回占位值
            ...
            private static void checkSubExpression(Stack<Type> operandStack) throws ComputeError {
                if (operandStack.size() != 1) {
                    throw new ComputeError(
                        BundleUtils.i18nHelper("表达式无效", "Invalid expression: too many operands"));
                }
            }
    ...
    public static class InvokeToken extends AbstractToken {
        ...
        public InvokeInfo parseForInvokeInfo(String token) {
            if (!new TokenFactory.InvokeRule().match(token) && !new TokenFactory.PureCallRule().match(token)) {
                throw new ComputeError(BundleUtils.i18n("debug.eval.parseFroInvokeInfo.error") + token);
            }
        ...
        protected Value doInvoke(Value callV, Method method, InvokeInfo info) {
            ...
            try {
                ...
                throw new ComputeError(BundleUtils.i18n("debug.eval.invokeMethod.error") + e);
            }
            // invokeMethod执行结束
            ctx.invokeMethodDone();
            return resultV;
        ...
        public Method getMethod(Value callV, InvokeInfo info) {
            ...
            if (!checkMethodExist(callVName, methodName, callV)) {
                throw new ComputeError(
                    callVName + " " + BundleUtils.i18n("debug.eval.not.exist") + " " + methodName + " "
                        + BundleUtils.i18n("debug.eval.method"));
            }
            // 判断重载
        ...
        private Method overloadCheck(Value callV, InvokeInfo info) {
            ...
            if (candidates.isEmpty()) {
                throw new ComputeError(
                    callVName + " " + BundleUtils.i18n("debug.eval.no.applicable.for.exp") + " " + methodName + " "
                        + BundleUtils.i18n("debug.eval.method"));
            }
            // 如果只有一个
            if (candidates.size() == 1) {
                ...
                // todo: 以后支持一下重载类型
                throw new ComputeError(BundleUtils.i18n("debug.eval.not.support.overload"));
            }
            return targetMethod;
        }
         ...
         */
        private void matchValueAndType(Method targetMethod, InvokeInfo info)
            throws ClassNotLoadedException, IncompatibleThreadStateException, InvocationException, InvalidTypeException {
            List<Value> params = info.params;
            ...
            if (!types.isEmpty() && (paramNames == null || paramNames.length != types.size())) {
                throw new ComputeError(
                    BundleUtils.i18n("debug.eval.method.parse.error") + info + " " + BundleUtils.i18n(
                        "debug.eval.actual.param") + types.stream().map(Type::name).collect(Collectors.joining(",")));
            }
         ...
         * @param type 方法的参数类型
         */
        private Value tryMatchValue(Value v, Type type, String vName)
            throws ClassNotLoadedException, IncompatibleThreadStateException, InvocationException, InvalidTypeException {
            /*
            ...
            if (!checkConsist(v, type)) {
                ...
                if (super.isObject(type)) {
                    if (v instanceof PrimitiveValue) {
                        ...
                        throw new ComputeError(
                            BundleUtils.i18n("debug.eval.variable.type.error") + vName + BundleUtils.i18n(
                                "debug.eval.type") + v.type().name() + ", " + BundleUtils.i18n("debug.eval.input.type")
                                + " " + type.name());
                    }
                } else {
                    // 报错
                    throw new ComputeError(
                        BundleUtils.i18n("debug.eval.variable.type.error") + vName + BundleUtils.i18n("debug.eval.type")
                            + v.type().name() + ", " + BundleUtils.i18n("debug.eval.input.type") + " " + type.name());
                }
            }
            return v;
        }
        ...
        private String getClassName(Type type) {
            ...
            throw new ComputeError(BundleUtils.i18n("action.leetcode.unknown.error") + " type: " + type.name());
        }
        ...
        private List<Value> getParams(String[] paramNames) {
            ...
            for (int i = 0; i < paramNames.length; i++) {
                try {
                    ...
                    LogUtils.warn(DebugUtils.getStackTraceAsString(e));
                    throw new ComputeError(
                        paramNames[i] + " " + BundleUtils.i18n("debug.eval.parse.error") + "! cause is "
                            + e.getMessage());
                }
            }
            return values;
    ...
    public static class PureCallTokenChain extends PureCallToken implements TokenChain {
        ...
        private String getPureCall(String token) {
            ...
            if (!matcher.find()) {
                throw new ComputeError(
                    "PureCallTokenChain" + BundleUtils.i18n("debug.eval.unrecognize") + " token: " + token);
            }
            int end = matcher.end();
        ...
        public Value getValue(@NotNull PValue pValue) {
            if (pValue.value == null) {
                throw new ComputeError(
                    BundleUtils.i18n("debug.eval.chain.pre.call") + pValue.preName + " = null, " + BundleUtils.i18n(
                        "debug.eval.chain.exp.error"));
            }
            String pureCall = getPureCall(token);
            ...
            if (!(pV instanceof ObjectReference)) {
                throw new ComputeError(BundleUtils.i18n("debug.eval.chain.exp.error") + " " + BundleUtils.i18n(
                    "debug.eval.only.object.has.method") + "  pV = " + pV.type().name());
            }
    ...
    public static class VariableTokenChain extends VariableToken implements TokenChain {
        ...
        private Value getVariableValue(String vName, PValue pValue) {
            ...
            if (!value.contain) {
                ...
                if (!value.contain) {
                    throw new ComputeError(
                        pValue.preName + " " + BundleUtils.i18n("debug.eval.not.exist") + " " + vName + "!");
                }
            }
            return value.value;
        }
    ...
    public static class ArrayToken extends AbstractToken {
        ...
        public ArrayReference getArrayValue(String arrayId) {
            ...
            if (!(v instanceof ArrayReference)) {
                throw new ComputeError(vName + BundleUtils.i18n("debug.eval.not.array"));
            }
        ...
        public List<Integer> getDims(String arrayId) {
            ...
            if (dimsStr.isEmpty()) {
                throw new ComputeError(BundleUtils.i18n("debug.eval.array.no.dim"));
            }
        ...
        public Value itrArray(ArrayReference array, List<Integer> dims, String arrayId) {
            ...
            for (int i = 0; i < dims.size(); i++) {
                int dim = dims.get(i);
                if (dim >= array.length()) {
                    if (I18nTypeEnum.getI18N(AppSettings.getInstance().getLocale()) == I18nTypeEnum.ZH) {
                        throw new ComputeError(
                            "数组 " + arrayId + " 访问越界越界!! 数组第" + (i + 1) + "维度的元素个数=" + array.length()
                                + ", 索引=" + dim);
                    } else {
                        throw new ComputeError(
                            "Array " + arrayId + " out of bounds!! The number of elements in the " + (i + 1)
                                + " dimension of the array=" + array.length() + ", index=" + dim);
                    }
                }
                if (dim <= -1) {
                    if (I18nTypeEnum.getI18N(AppSettings.getInstance().getLocale()) == I18nTypeEnum.ZH) {
                        ...
                        throw new ComputeError("Array index must be a positive number!! index = " + value);
                    }
                }
                ...
                if (i != dims.size() - 1) {
                    if (!(value instanceof ArrayReference)) {
                        if (I18nTypeEnum.getI18N(AppSettings.getInstance().getLocale()) == I18nTypeEnum.ZH) {
                            throw new ComputeError(
                                "变量" + arrayId + "第 " + i + " 维度不是数组类型, 是" + value.type().name()
                                    + "类型!!");
                        } else {
                            throw new ComputeError(
                                "Variable " + arrayId + " dimension " + i + " is not an array type, it is "
        ...
        private String getArrayIdentify(String token) {
            ...
            if (!matcher.find()) {
                if (I18nTypeEnum.getI18N(AppSettings.getInstance().getLocale()) == I18nTypeEnum.ZH) {
                    ...
                    throw new ComputeError("Unable to match array, ArrayChainRule rule error! " + token);
                }
            }
            int end = matcher.end();
            return token.substring(0, end);
    ...
    public static class ArrayTokenChain extends ArrayToken implements TokenChain {
        ...
        public boolean isLastChain(String token) {
            ...
            String array = flag ? token.substring(0, CharacterHelper.matchArrayBracketSafe(token, 0))
                : super.getArrayIdentify(token); // 判断是否为隐式调用
        ...
        public Value getValue(PValue pValue) {
            if (pValue.value == null) {
                if (I18nTypeEnum.getI18N(AppSettings.getInstance().getLocale()) == I18nTypeEnum.ZH) {
                    ...
                    throw new ComputeError(
                        "The call before the chain call " + pValue.preName + " is null, chain expression error!");
                }
            }
            boolean flag = isImplicitCall(token);
            String arrayId = flag ? token.substring(0, CharacterHelper.matchArrayBracketSafe(token, 0))
                : super.getArrayIdentify(token); // 判断是否为隐式调用
            Value pV = pValue.value;
            ...
            if (flag) {
                if (!(pV instanceof ArrayReference)) {
                    if (I18nTypeEnum.getI18N(AppSettings.getInstance().getLocale()) == I18nTypeEnum.ZH) {
                        ...
                        throw new ComputeError(
                            "pV is not Array type! Expression calculation result error! type = " + pV.type().name());
                    }
                }
            } else {
                // 非隐式调用, 从pV中取值
                ...
                if (!value.contain) {
                    if (I18nTypeEnum.getI18N(AppSettings.getInstance().getLocale()) == I18nTypeEnum.ZH) {
                        ...
                        throw new ComputeError("Variable " + arrayId + " is not defined!");
                    }
                }
                pV = value.value;
            }
    ...
    public static class ConstantToken extends AbstractToken {
        ...
        public Value getValue() {
            ...
            if (I18nTypeEnum.getI18N(AppSettings.getInstance().getLocale()) == I18nTypeEnum.ZH) {
                ...
                throw new IllegalArgumentException("Unrecognized constant type: " + token);
            }
        }
    }
    ...
    public static class TokenFactory {
        ...
        public Token parseToToken(String s, Context ctx) {
            ...
            if (matches.isEmpty()) {
                if (I18nTypeEnum.getI18N(AppSettings.getInstance().getLocale()) == I18nTypeEnum.ZH) {
                    ...
                    throw new ComputeError("Unrecognized content! " + s);
                }
            }
            if (matches.size() > 1) {
                if (I18nTypeEnum.getI18N(AppSettings.getInstance().getLocale()) == I18nTypeEnum.ZH) {
                    throw new ComputeError(
                        s + " 被多条规则匹配! 请检查规则编写是否正确! + " + matches.stream().map(Rule::getName)
                            .collect(Collectors.joining(",")));
                } else {
                    throw new ComputeError(
                        s + " is matched by multiple rules! Please check whether the rule writing is correct! + "
                            + matches.stream().map(Rule::getName).collect(Collectors.joining(",")));
                }
            }
            ...
            try {
                ...
                if (I18nTypeEnum.getI18N(AppSettings.getInstance().getLocale()) == I18nTypeEnum.ZH) {
                    ...
                    throw new ComputeError(
                        className + "Instantiation error, the object is an abstract class and cannot be instantiated! "
                            + e.getMessage());
                }
            } catch (IllegalAccessException e) {
                LogUtils.error(e);
                if (I18nTypeEnum.getI18N(AppSettings.getInstance().getLocale()) == I18nTypeEnum.ZH) {
                    throw new ComputeError(className + "构造函数访问错误, String.class, Context.class的构造函数无权访问! "
                        + e.getMessage());
                } else {
                    throw new ComputeError(
                        className + "Constructor access error, String.class, Context.class Constructor has no access! "
                            + e.getMessage());
                }
            } catch (InvocationTargetException e) {
                LogUtils.error(e);
                if (I18nTypeEnum.getI18N(AppSettings.getInstance().getLocale()) == I18nTypeEnum.ZH) {
                    throw new ComputeError(className + "构造函数执行异常, String.class, Context.class的运行过程中发生报错! "
                        + e.getMessage());
                } else {
                    throw new ComputeError(className
                        + "Constructor execution exception, an error occurred during the execution of String.class, Context.class! "
                        + e.getMessage());
                }
                ...
                if (I18nTypeEnum.getI18N(AppSettings.getInstance().getLocale()) == I18nTypeEnum.ZH) {
                    throw new ComputeError(
                        className + "没有符合String.class, Context.class的构造函数! " + e.getMessage());
                } else {
                    throw new ComputeError(
                        className + "There is no constructor that matches String.class, Context.class! "
                            + e.getMessage());
                }
            }
        }
                ...
                if (c == '(') {
                    ...
                    if (stack.isEmpty()) {
                        ...
                        if (I18nTypeEnum.getI18N(AppSettings.getInstance().getLocale()) == I18nTypeEnum.ZH) {
                            ...
                            throw new ComputeError("Expression exception, bracket mismatch!");
                        }
                    }
                    stack.pop();
        ...
        public Token parseToTokenFromStart(String token, Context ctx) {
            ...
            if (hits.size() == 1) {
                ...
                if (hits.stream().anyMatch(e -> e.rule instanceof EvalRule)) {
                    ...
                    if (targetHit == null) {
                        if (I18nTypeEnum.getI18N(AppSettings.getInstance().getLocale()) == I18nTypeEnum.ZH) {
                            throw new ComputeError(
                                s + " 被EvalRule重复匹配两次! 请检查规则编写是否正确! + " + hits.stream()
                                    .map(e -> e.rule.getName()).collect(Collectors.joining(",")));
                        } else {
                            throw new ComputeError(
                                s + " is matched by EvalRule twice! Please check the rule writing is correct! + "
                                    + hits.stream().map(e -> e.rule.getName()).collect(Collectors.joining(",")));
                        }
                    }
                } else {
                    if (I18nTypeEnum.getI18N(AppSettings.getInstance().getLocale()) == I18nTypeEnum.ZH) {
                        throw new ComputeError(
                            s + " 被两条规则匹配! 请检查规则编写是否正确! + " + hits.stream().map(e -> e.rule.getName())
                                .collect(Collectors.joining(",")));
                    } else {
                        throw new ComputeError(
                            s + " is matched by two rules! Please check the rule writing is correct! + " + hits.stream()
                                .map(e -> e.rule.getName()).collect(Collectors.joining(",")));
                    }
                }
            } else {
                if (I18nTypeEnum.getI18N(AppSettings.getInstance().getLocale()) == I18nTypeEnum.ZH) {
                    throw new ComputeError(
                        s + " 被多条规则匹配! 请检查规则编写是否正确! + " + hits.stream().map(e -> e.rule.getName())
                            .collect(Collectors.joining(",")));
                } else {
                    throw new ComputeError(
                        s + " is matched by multiple rules! Please check the rule writing is correct! + "
                            + hits.stream().map(e -> e.rule.getName()).collect(Collectors.joining(",")));
                }
            }
            ...
            try {
                ...
                if (I18nTypeEnum.getI18N(AppSettings.getInstance().getLocale()) == I18nTypeEnum.ZH) {
                    ...
                    throw new ComputeError(
                        className + "instantiation error, the object is an abstract class, cannot be instantiated! "
                            + e.getMessage());
                }
            } catch (IllegalAccessException e) {
                LogUtils.error(e);
                ...
                if (I18nTypeEnum.getI18N(AppSettings.getInstance().getLocale()) == I18nTypeEnum.ZH) {
                    ...
                    throw new ComputeError(className
                        + "Constructor execution exception, String.class, Context.class is running and an error occurs! "
                        + e.getMessage());
                }
            } catch (NoSuchMethodException e) {
                LogUtils.error(e);
                if (I18nTypeEnum.getI18N(AppSettings.getInstance().getLocale()) == I18nTypeEnum.ZH) {
                    ...
                    throw new ComputeError(
                        className + "does not have a constructor that matches String.class, Context.class! "
                            + e.getMessage());
                }
            }
        }
        ...
        public TokenChain parseToTokenChain(String s, Context ctx) {
            ...
            if (matches.stream().anyMatch(e -> e instanceof EvalRule)) {
                if (I18nTypeEnum.getI18N(AppSettings.getInstance().getLocale()) == I18nTypeEnum.ZH) {
                    throw new ComputeError(
                        s + " 被EvalRule匹配! 无法得到对应的链式! parseToTokenChain方法使用错误! + " + matches.stream()
                            .map(Rule::getName).collect(Collectors.joining(",")));
                } else {
                    throw new ComputeError(s
                        + " is matched by EvalRule! Cannot get the corresponding chain! parseToTokenChain method error! + "
                        + matches.stream().map(Rule::getName).collect(Collectors.joining(",")));
                }
            }
            Rule targetRule;
            ...
            if (!(matches.get(0) instanceof RuleChain)) {
                ...
                if (!ruleMap.containsKey(key)) {
                    if (I18nTypeEnum.getI18N(AppSettings.getInstance().getLocale()) == I18nTypeEnum.ZH) {
                        throw new ComputeError(s + " 被" + key + "匹配! 无法得到对应的链式! parseToTokenChain方法使用错误! + "
                            + matches.stream().map(Rule::getName).collect(Collectors.joining(",")));
                    } else {
                        throw new ComputeError(s + " is matched by " + key
                            + "! Cannot get the corresponding chain! parseToTokenChain method error! + "
                            + matches.stream().map(Rule::getName).collect(Collectors.joining(",")));
                    }
                }
            ...
            try {
                ...
                if (I18nTypeEnum.getI18N(AppSettings.getInstance().getLocale()) == I18nTypeEnum.ZH) {
                    ...
                    throw new ComputeError(className
                        + "Constructor access error, String.class, Context.classConstructor cannot be accessed! "
                        + e.getMessage());
                }
            } catch (InvocationTargetException e) {
                LogUtils.error(e);
                if (I18nTypeEnum.getI18N(AppSettings.getInstance().getLocale()) == I18nTypeEnum.ZH) {
                    ...
                    throw new ComputeError(className
                        + "Constructor execution exception, an error occurred during the running of String.class, Context.class! "
                        + e.getMessage());
                }
            } catch (NoSuchMethodException e) {
                LogUtils.error(e);
        ...
        public static abstract class AbstractRule implements Rule {
            ...
            public static final Pattern constantPattern = Pattern.compile(
                "\\d+(\\.\\d+)?|\"([^\"\\\\]|\\\\.)*\"|'([^'\\\\]|\\\\.)*'");
                ...
                if (c == '"') {
                    ...
                    if (j == arr.length) {
                        if (I18nTypeEnum.getI18N(AppSettings.getInstance().getLocale()) == I18nTypeEnum.ZH) {
                            ...
                            throw new ComputeError(
                                "Expression error, the string type expression must contain a matching \"" + s);
                        }
                    }
                    // j 指向", 因此需要j + 1
                    ...
                    if (j == arr.length) {
                        if (I18nTypeEnum.getI18N(AppSettings.getInstance().getLocale()) == I18nTypeEnum.ZH) {
                            ...
                            throw new ComputeError(
                                "Expression error, the character type expression must contain a matching ' " + s);
                        }
                    }
                    // j指向', 因此需要j + 1
                    return s.substring(0, j + 1);
                        ...
                        if (!CharacterHelper.isArabicNumber(arr[j])) {
                            ...
                            if (arr[j] != '.') {
                                ...
                                if (I18nTypeEnum.getI18N(AppSettings.getInstance().getLocale()) == I18nTypeEnum.ZH) {
                                    ...
                                    throw new ComputeError(
                                        "Expression error, the numeric type expression is not allowed to appear two decimal points "
                                            + s);
                                }
                            }
                        }
