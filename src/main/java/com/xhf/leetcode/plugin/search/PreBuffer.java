...
/**
 * 预加载缓冲区, 配合快照迭代器{@link com.xhf.leetcode.plugin.search.SourceManager.CaptureIterator}使用.
 * {@link PreBuffer}(简称PB)维护preBuffer数组, 以及缓冲区内有效数据长度, PB的有效性
 * <p>
 * PB是为了解决capture iterator(简称ci)在迭代边界数据时, 遇到诸多问题而引入的解决方案
 * PB的出现允许ci在不修改底层{@link SourceBuffer}(简称SB)内部数据的情况下, 让ci具有迭代后续分段数据的能力
 * <p>
 * 与此同时, PB的出现也带来了全新的问题: 如果{@link SourceManager}将数据源source的数据写入preBuffer, 那么下一轮读取, source的偏移量将发生改变
 ...
 * [一, 如, 既, 往, a]
 *  @
 * 当前SB迭代器处理到'一'字, 按照最长匹配原则, SB会持续迭代后续字符, 直到遇到'a', 则会终止匹配
 ...
 * <p>
 * 另外, PB内部维护的buffer大小和SB内部维护的buffer大小一致. 并且大小位于[SourceManager.MIN_BUFFER_SIZE, SourceManager.DEFAULT_BUFFER_SIZE]
 */
class PreBuffer {
