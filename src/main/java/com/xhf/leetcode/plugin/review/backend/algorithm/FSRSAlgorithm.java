...
public class FSRSAlgorithm {
    ...
    private final float[] WEIGHTS = new float[]{1F, 1F, 5F, -1F, -1F, 0.1F, 1.5F, -0.2F, 0.8F, 2, -0.2F, 0.2F,
        1F}; /* 各参数权重配置 */
    ...
    private final int scheduledDays;
    ...
    public FSRSAlgorithmResult calc() {
        this.card = new SchedulingCard(System.currentTimeMillis(), this.stability, this.difficulty, this.elapsedDays,
            this.scheduledDays, this.repetitions, this.state, this.lastReview);
        // 为每个评分枚举创建一张卡片
        for (FSRSRating rating : FSRSRating.values()) {
            this.card.getRatingToCard().put(rating,
                new SchedulingCard(System.currentTimeMillis(), this.stability, this.difficulty, this.elapsedDays,
                    this.scheduledDays, this.repetitions, FSRSState.NEW, this.lastReview));
        }

        if (this.card.getState() == FSRSState.NEW) {
            this.card.setElapsedDays(0);
        } else {
            this.card.setElapsedDays(
                (int) (TimeUnit.MILLISECONDS.toDays(this.card.getDueTime() - this.card.getLastReview())));
        }
        ...
        if (this.card.getState() == FSRSState.NEW) {
            ...
            this.card.getRatingToCard().get(FSRSRating.AGAIN)
                .setDueTime(this.card.getDueTime() + TimeUnit.MINUTES.toMillis(1));
            this.card.getRatingToCard().get(FSRSRating.HARD)
                .setDueTime(this.card.getDueTime() + TimeUnit.MINUTES.toMillis(5));
            this.card.getRatingToCard().get(FSRSRating.GOOD)
                .setDueTime(this.card.getDueTime() + TimeUnit.MINUTES.toMillis(10));
            this.card.getRatingToCard().get(FSRSRating.EASY)
                .setDueTime(this.card.getDueTime() + TimeUnit.MINUTES.toMillis(15));
            ...
            nextInterval = this.nextInterval(
                this.card.getRatingToCard().get(this.rating).getStability() * this.getBonusByRating(this.rating));
            this.card.getRatingToCard().get(this.rating).setScheduledDays(nextInterval);
            this.card.getRatingToCard().get(this.rating)
                .setDueTime(this.card.getDueTime() + TimeUnit.DAYS.toMillis(nextInterval));
            ...
            goodInterval = Math.max(this.nextInterval(this.card.getRatingToCard().get(FSRSRating.GOOD).getStability()),
                hardInterval + 1);
            easyInterval = Math.max(
                this.nextInterval(this.card.getRatingToCard().get(FSRSRating.EASY).getStability() * EASY_BONUS),
                goodInterval + 1);
            ...
            easyInterval = Math.max(
                this.nextInterval(this.card.getRatingToCard().get(FSRSRating.EASY).getStability() * HARD_FACTOR),
                goodInterval + 1);
            this.card.schedule(hardInterval, goodInterval, easyInterval);
        }
    ...
    private void next(float lastDifficulty, float lastStability, float retrievability) {
        for (FSRSRating rating : FSRSRating.values()) {
            ...
            if (rating == FSRSRating.AGAIN) {
                this.card.getRatingToCard().get(rating).setStability(
                    this.nextForgetStability(this.card.getRatingToCard().get(rating).getDifficulty(), lastStability,
                        retrievability));
            } else {
                this.card.getRatingToCard().get(rating).setStability(
                    this.nextRecallStability(this.card.getRatingToCard().get(rating).getDifficulty(), lastStability,
                        retrievability));
            }
        }
    }
    ...
    private float nextForgetStability(float difficulty, float stability, float retrievability) {
        return (float) (WEIGHTS[9] * Math.pow(difficulty, WEIGHTS[10]) * Math.pow(stability, WEIGHTS[11]) * Math.exp(
            (1 - retrievability) * WEIGHTS[12]));
    }
